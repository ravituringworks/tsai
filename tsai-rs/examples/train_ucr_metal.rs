//! Example: Train InceptionTimePlus on UCR Dataset with Metal GPU
//!
//! This example demonstrates GPU-accelerated training using tsai-rs with Metal backend:
//! - Loading UCR time series datasets
//! - Creating dataloaders with shuffling
//! - Training InceptionTimePlus with OneCycleLR on Metal GPU
//! - Comparing performance vs CPU
//!
//! Run with: cargo run --example train_ucr_metal --release --features backend-wgpu
//!
//! For a quick test, use a small dataset like ECG200:
//!   cargo run --example train_ucr_metal --release --features backend-wgpu -- --dataset ECG200 --epochs 10
...
/// Backend type for training (WGPU with autodiff - uses Metal on macOS).
...
fn main() -> Result<()> {
    ...
    println!("==============================================================");
    println!("       tsai-rs: Time Series Classification (Metal GPU)        ");
    println!("==============================================================");
    println!();
    ...
    // =========================================================================
    // Step 4: Initialize Model and Trainer with Metal GPU
    // =========================================================================
    println!("Step 4: Initializing Training (Metal GPU)");
    println!("-------------------------------------------------------------");

    // Use default WGPU device - will select Metal on macOS
    let device = WgpuDevice::default();
    println!("  Device: {:?}", device);
    println!("  Backend: WGPU (Metal on macOS, Vulkan on Linux/Windows)");
    ...
    // Step 5: Train!
    // =========================================================================
    println!("Step 5: Training on Metal GPU");
    println!("-------------------------------------------------------------");
    println!();
    ...
    println!();
    println!("  Dataset: {}", dataset_name);
    println!("  Backend: Metal GPU (WGPU)");
